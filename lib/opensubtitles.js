...
os.prototype.searchSubtitles = function(query, cb) {
    var self = this;
    this.api.LogIn(function(err, res) {
        if(err) return cb(err);

        var token = res.token;
        self.api.SearchSubtitles(function(err, res){
...
os.prototype.computeStreamHash = function(strem_start, stream_end, file_size, cb) {
    var self = this;
    var t_chksum = [];
    ...
    var streams = [strem_start, stream_end];

    var calcuate_checksum = function() {
        ...
        stream.on('end', function(){
            ...
            self.emit('checksum-ready', self.checksumBuffer(buffer, 8), "buf?");
        });
